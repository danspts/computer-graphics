...
def render_scene(camera, ambient, lights, objects, screen_size, max_depth, render_model:RenderModel= RenderModel.default):
    ...
    match render_model:
        case RenderModel.blinn_phong:
            model_func = lambda n, L, V: n @ normalize(L + V)
        case RenderModel.phong:
            model_func = lambda n, L, V: L @ V
...
def ray_trace(ray: Ray, ambient: Tuple[float, float, float], lights: List[LightSource], objects: List[Object3D], max_depth: int, model_func: Callable):
    color = np.zeros(3)
    if max_depth <= 0:
        return color
    ...
    for light in lights:
        ...
        color += light.get_intensity(_P) * (L @ n * obj.diffuse + model_func(n, L, V) ** obj.shininess * obj.specular)
    return (
        color
        + obj.reflection * ray_trace(Ray(_P, V), ambient, lights, objects, max_depth - 1, model_func) if obj.reflection else 0
        + obj.refraction * ray_trace(ray.calc_refraction(obj, _P, n), ambient, lights, objects, max_depth - 1, model_func) if obj.refraction else 0
    )
